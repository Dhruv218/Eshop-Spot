              ...
              {data.map(item => {
                ...
                return (
                  <tr className="t-r">
                    ...
                    {item.no === '' || item.no[0] === '' ? <td> <input type='number' min={0} className="quantity" defaultValue={item.no = 1} onChange={e => {
                      setAmount(e.target.value);
                      item.no = amount;
                    }}
                    /></td> :
                    <td><input className="quantity" type='number' min={0} defaultValue={item.no} onChange={e => {
                        setAmount(e.target.value);
                        item.no = amount;
                      }}
                      /></td>
